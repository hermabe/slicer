...
std::vector<Edge3d> intersection(const Plane & plane, const Triangle3d & triangle)
{
	...
	std::function<unsigned int(std::array<int, 3>&)> indexOfInPlane = [](std::array<int, 3> & arr) {
...
std::vector<Triangle3d>withVertex(const std::vector<Triangle3d> & triangles, const Vector3d & vertex)
{
	...
	std::copy_if(triangles.cbegin(), triangles.cend(), result.begin(), [&](const Triangle3d & triangle) {
		return std::any_of(triangle.vertices.cbegin(), triangle.vertices.cend(), [&](const Vector3d & triangle_vertex) {
...
bool aboveOrBelow(const Plane & plane, const Triangle3d & triangle) {
	...
	std::function<bool(const Vector3d & vertex)> above = [&](const Vector3d & vertex) { return planeCosine(plane, vertex) > 0; };
	std::function<bool(const Vector3d & vertex)> below = [&](const Vector3d & vertex) { return planeCosine(plane, vertex) < 0; };
...
std::vector<Triangle3d> intersects(const std::vector<Triangle3d> & triangles, const Plane & plane)
{
	...
	auto it = std::copy_if(triangles.cbegin(), triangles.cend(), result.begin(), [&](const Triangle3d & triangle) {
